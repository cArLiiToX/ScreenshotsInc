                        ...
                        foreach ($variations->attributes as $attribute) {
                            if ($attribute->name == 'xe_color') {
                                if (empty($variant) || (!empty($variant) && !in_array($attribute->option, $color_array))) {
                                    ...
                                    $term_id = $wpdb->get_var("SELECT t.term_id FROM $table_name t LEFT JOIN $table_name1 tt ON (t.term_id = tt.term_id) WHERE tt.taxonomy = 'pa_xe_color' AND (t.slug='".$color_option."' OR lower(t.name) = '".strtolower($color_option)."')");
                    ...
                    foreach ($result->product->variations as $variants) {
                        if ($variants->id == $pid) {
                            ...
                            foreach ($pvariant->attributes as $attribute) {
                                ...
                                if ($attribute->option != "") {
                                    $productsArr['attributes'][$attribute->name] = ($attribute->name == 'xe_size')?strtoupper($attribute->option):$attribute->option;
