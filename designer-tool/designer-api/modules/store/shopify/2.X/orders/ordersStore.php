...
class OrdersStore extends UTIL
{
    ...
    public function getOrders()
    {
        ...
        if ($this->storeApiLogin == true) {
            ...
            $start = (isset($this->_request['start']) && trim($this->_request['start']) != '') ? trim($this->_request['start']) : '';
            try {
                $filters = array('lastOrderId' => $lastOrderId, 'store' => $this->getDefaultStoreId(), 'range' => $range, 'fromDate' => $fromDate, 'toDate' => $toDate, 'start' => $start);
...
//for name and number folder scan
            ...
            if (count($itemArray) > 0) {
                ...
                foreach ($response['line_items'] as $lt) {
                    ...
                    foreach ($prodJson['product']['options'] as $option) {
                        ...
                        } elseif (strtolower($option['name']) !== 'color' || strtolower($option['name']) !== 'size' || strtolower($option['name']) !== 'quantity' || strtolower($option['name']) !== 'title') {
                    ...
                    $sku = substr($lt['sku'], (strpos($lt['sku'], '_') + 1));
                    ...
                    if ($ref_id) {
                        ...
                        if (!empty($extraAtt) && !in_array('Title', $extraAtt)) {
                            foreach ($extraAtt as $key => $extraAtr) {
                        ...
                        if (empty($json_content['nameNumberData'])) {
                            ...
                            if ($sizePos != '') {
                                $html .= '<td style = "border: 1px solid #ccc; border-collapse: collapse; padding: 8px; text-align: left; color: #333;">' . $Attrs[($sizePos - 1)] . '</td>';
                            }
                        }
                        if ($colorPos != '') {
                        ...
                        if (!empty($extraAtt) && !in_array('Title', $extraAtt)) {
                            foreach ($extraAtt as $key => $extraAtr) {
                                $pos = $key - 1;
                                $html .= '<td style = "border: 1px solid #ccc; border-collapse: collapse; padding: 8px; text-align: left; color: #333;">' . $Attrs[$pos] . '</td>';
                            }
                        }
                        // Hide product wise print method Multiple Boundary
                        if ($multipleBoundary != 'true') {
    ...
    public function manageInventory($orderID)
    {
       $result = $this->storeApiLogin();
        if ($this->storeApiLogin == true) {
            $key = $GLOBALS['params']['apisessId'];
            $$filters = array('orderID' => $orderID);
            $order = $this->proxy->call($key, 'cedapi_cart.manageInventory', $$filters);
        } 
    }
}
