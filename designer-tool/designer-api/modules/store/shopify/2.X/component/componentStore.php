...
$dir = strstr(getcwd(), '/designer-tool/', true);
require_once $dir . "/shopify/lib/config.php";
class ComponentStore
{
    ...
    public function storeApiLogin()
    {
        ...
        try {
            $this->proxy = new Shopify_api();
        } catch (Exception $e) {
            $result = json_encode(array('isFault' => 1, 'initFaultMessage' => $e->getMessage()));
            $error = true;
        }
        if (!$error) {
            $sessionId = $this->proxy->login(APIUSER, APIPASS);
            if ($sessionId) {
                $GLOBALS['params']['apisessId'] = html_session_set('apisessId', $sessionId);
            }
            $key = $GLOBALS['params']['apisessId'];
            if ($key) {
                $this->storeApiLogin = true;
            } else {
                $result = json_encode(array('isFault' => 1, 'loginFaultMessage' => "Authentication Failed. Please set api user name and password."));
                $error = true;
            }
        }
        return $result;
    ...
    public function getUserImages()
    {
        ...
        if ($this->isValidCall($apiKey)) {
            try {
                ...
                if ($uid != "" || $uid != '0') {
                    $sql = "Select image,thumbnail,type,customer_id,uid from  " . TABLE_PREFIX . "image_data where uid='" . $uid . "'" . " and customer_id ='" . $customerId . "'";
                }
