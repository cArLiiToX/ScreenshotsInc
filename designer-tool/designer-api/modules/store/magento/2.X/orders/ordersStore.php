...
class OrdersStore extends UTIL
{
    ...
    public function getOrderDetails($return = 0)
    {
        ...
        if ($this->storeApiLogin == true) {
            ...
            try {
                $filters = array('orderId' => $orderId, 'store' => $this->getDefaultStoreId(), 'color' => $this->getStoreAttributes("xe_color"), 'size' => $this->getStoreAttributes("xe_size"));
    ...
    public function downloadOrdersZipApp()
    {
        ...
        if ($this->storeApiLogin == true) {
            ...
            try {
                ...
                $result = $this->apiCall('Cart', 'orderIdFromStore', $filters);
...
//for name and number folder scan
    ...
    public function downloadOrderDetail()
    {
        ...
        if (!empty($result)) {
            ...
            $xe_size = ucfirst($this->getStoreAttributes("xe_size"));
            $xe_color = ucfirst($this->getStoreAttributes("xe_color"));
            ...
            $langSize = (!empty($languageJson1['SIZE'])) ? $xe_size : $xe_size;
            $langColor = (!empty($languageJson1['COLOR'])) ? $xe_color : $xe_color;
            ...
            $colorincrment = 1;
            $html = '<html><title>Order APP</title><style>body,p,td,tr,th,h1,h2,h3,h4,h5,h6{font-family:arial;-webkit-print-color-adjust:exact; }h1,h2,h3,h4,h5,h6{margin:0px; padding:0px; font-weight:normal;}.wrapper{width: 1024px; margin:auto;}.customer-info{clear:both;height:100px; background-color:; border-bottom:1px solid #ccc; margin-bottom: 20px;}.customer-details{float:left; padding:20px 0px 20px 0px; }    .order-code{float:right; padding:30px 20px 0px 0px;}.size-30{font-size:30px;}.size-25{font-size:25px;}.size-20{font-size:20px;} .size-17{font-size:17px;}.size-15{font-size:15px;}.size-12{font-size:12px;}.dark{color:#333;}.grey2{color:#ababab;}.grey{color:#ccc;}.light{color:#e6e6e6;}.items{border: 2px solid #4CAF50; margin-bottom:20px; border-radius:5px;; width:1200px;}.product-name{background-color:#4CAF50; padding:15px;border-radius:2px 2px 0px 0px; font-size:20px; color:#fff; -webkit-print-color-adjust: exact;}      .product-desc{padding:20px; background-color:#efefef; margin:0px 20px 20px 20px;}.product-desc-2{padding:0px 20px 20px 20px;width:44%; text-align: center; float:left; background-color:#efefef; margin:20px 0px 20px 20px;min-height:810px;}.barcode{margin-right:50px; margin-top:20px;}.barcode img{width:100%; height:50px;}.m-b-5{margin-bottom:5px;}.m-b-7{margin-bottom:7px;}.m-b-10{margin-bottom:10px;}.m-b-15{margin-bottom:15px;}.m-b-20{margin-bottom:20px;}.m-b-25{margin-bottom:25px;}.padding{padding:20px;}.product{margin-bottom:20px; min-height:400px; min-width:400px;}table {width:100%;}table, th, td {border: 1px solid #ccc; border-collapse: collapse; }th, td {padding: 8px;text-align: left; color:#333; }table.t01 tr:nth-child(even){background-color: #efefef;}table.t01 tr:nth-child(odd) {background-color:#efefef;}table.t01 th {background-color: #666;color: white;}.table-scroll{height: 141px;overflow: auto;}.notes{border: 0px;width: 100%;resize: vertical;padding-left: 15px;font-family: inherit;}.notes-padding{padding: 0px 20px 0px 20px;};</style><style type="text/css" media="print">.table-scroll{ height: auto!important;overflow: visible!important; }</style><body><div style="width: 1024px; margin:auto;">';
            $generatorPNG = new Picqer\Barcode\BarcodeGeneratorPNG();
            if ($langSize == 'Xe_size') {
                $langSize = 'Size';
            }
            if ($langColor == 'Xe_color') {
                $langColor = 'Color';
            }
            $html .= '<div style="clear:both;height:100px; background-color:; border-bottom:1px solid #ccc; margin-bottom: 20px;">
                        <div style="float:left; padding:20px 0px 20px 0px;">
                            <h1 style="font-size:20px;">' . $value->billing_address->first_name . ' ' . $value->billing_address->last_name . '</h1><h2 style="margin:0px; padding:0px; font-weight:normal;font-size: 17px;">' . $langOrderId . ': <span style="color:#333;">' . $value->order_incremental_id . '</span></h2> <h3 style="margin:0px; padding:0px; font-weight:normal;font-size: 15px;">' . $langOrderDate . ': ' . gmdate('Y-m-d', strtotime($value->order_date)) . '&nbsp;<span style="color:#ababab;">(YYYY-MM-DD)</span> &nbsp; &nbsp;' . $langOrderTime . ': ' . date('H:i:s', strtotime($value->order_date)) . '</h3>
                        </div>
            ...
            foreach ($value->order_items as $line_key => $line_items) {
                ...
                if ($line_items->ref_id != '' && $line_items->ref_id != 0) {
                    ...
                    if (!empty($productData)) {
                        $html .= '<div style="border: 2px solid #4CAF50; margin-bottom:20px; border-radius:5px;display:block; width:100%;page-break-after:always;"><div style="background-color:#4CAF50; padding:15px;border-radius:2px 2px 0px 0px; font-size:20px; color:#fff; -webkit-print-color-adjust: exact;">' . $productname . '</div>
                         <div style="padding:20px;"><table style="border: 1px solid #ccc; border-collapse: collapse; width: 100%;"><tbody><tr>';
                        //Hide heading of quantity for name and number case
                        ...
                        foreach ($productData as $key => $pattribute) {
                            foreach ($attributes as $attribute) {
                                ...
                                if ($attributeCode == $key) {
                                    ...
                                    //Hide size attribute for name and number case
                                    if (!empty($json_content['nameNumberData'])) {
                                        if (strtolower($attributeCode) != strtolower($langSize)) {
                        ...
                        $html .= '</tr><tr>';
                        //Hide quantity value for name and number case
                        if (empty($json_content['nameNumberData'])) {
                        ...
                        foreach ($productData as $key => $pattribute) {
                            foreach ($attributes as $attribute) {
                                ...
                                if ($attributeCode == $key) {
                                    ...
                                    //Hide value of size attribute for name and number case
                                    if (!empty($json_content['nameNumberData'])) {
                                        if (strtolower($attributeCode) != strtolower($langSize)) {
                        ...
                        $html .= '<div style="border: 2px solid #4CAF50; margin-bottom:20px; border-radius:5px; display:block; width:100%;page-break-after:always;"><div style="background-color:#4CAF50; padding:15px;border-radius:2px 2px 0px 0px; font-size:20px; color:#fff; -webkit-print-color-adjust: exact;">' . $productname . '</div><div style="padding:20px;"><table style="border: 1px solid #ccc; border-collapse: collapse; width: 100%;"><tbody><tr>';
                        //Hide heading of size attribute and quantity for name and number case
                        if (empty($json_content['nameNumberData'])) {
                            $html .= '<th style="border: 1px solid #ccc; border-collapse: collapse;padding: 8px;text-align: left; color:#333;">' . $langQuantity . '</th>';
                            if ($size != 'No' && $size != '') {
                                $html .= '<th style="border: 1px solid #ccc; border-collapse: collapse;padding: 8px;text-align: left; color:#333;">' . $langSize . '</th>';
                            }
                        }
                        if ($color != 'No' && $color != '') {
                        ...
                        if (empty($json_content['nameNumberData'])) {
                            $html .= '<td style = "border: 1px solid #ccc; border-collapse: collapse; padding: 8px; text-align: left; color: #333;">' . $quantity . '</td>';
                            if ($size != 'No' && $size != '') {
                                $html .= '<td style = "border: 1px solid #ccc; border-collapse: collapse; padding: 8px; text-align: left; color: #333;">' . $size . '</td>';
                            }
                        }
                        if ($color != 'No' && $color != '') {
                            $html .= '<td style = "border: 1px solid #ccc; border-collapse: collapse; padding: 8px; text-align: left; color: #333;">' . $color . '</td>';
                        }
                        // Hide product wise print method Multiple Boundary
                        if ($multipleBoundary != 'true') {
                            $html .= '<td style = "border: 1px solid #ccc; border-collapse: collapse; padding: 8px; text-align: left; color: #333;">' . $printType . '</td>';
                    ...
                    foreach ($json_content['sides'] as $key => $sides) {
                            ...
                            $html .= '<div style="padding: 0px 20px 20px 20px; width: 44%; text-align: center; background-color: #efefef; margin: 20px 0px 20px 13px; display:inline-block; vertical-align:top;' . $onesidewidth . '"><div class="m-b-7 barcode"><img style="' . $barcodewidth . '" src="data:image/png;base64,' . $product_barcode . '" height="50px" alt="" /></div><div style="margin-bottom:10px;"><h3>' . $base64 . '</h3></div><div style="margin-bottom:20px; min-height:400px; min-width:400px;"><img style="width:100%;" class="product-img" src="' . $pUrl . '" alt=""></div>';
                            if (isset($sides['printSize']) && $sides['printSize'] != '' && $multipleBoundary != 'true') {
                                ...
                                $html .= '<div><h2 style="margin-bottom:10px; min-height:30px;">' . $langPrintSize . ':   ' . $printSize . '</h2></div>';
                            } else {
                                $html .= '<div style="margin-bottom:10px; min-height:30px;"> &nbsp; </div>';
                            }
                            $printColorNames = (isset($sides['printColorNames'])) ? count($sides['printColorNames']) : 0;
                            $height = $printColorNames * 30;
                            $html .= '<div class="table-scroll" style="height: 141px;overflow: auto;">';
                            ...
                            foreach ($sides['multiBound'] as $section => $sectionDetails) {
                                ...
                                if (isset($sides['printSize']) && $sides['printSize'] != '') {
                                    $html .= '<div class="table-scroll" style="height: 141px;overflow: auto;"><table style="width: 100%; height:' . $height . 'px" class="repeattd t01"><tbody><div style=" text-align: center; padding-top: 55px;"> &nbsp; </div></tbody></tbody></table></div>';
                                } else {
                                    $html .= '<div class="table-scroll" style="height: 141px;overflow: auto;"><table style="width: 100%; height:' . $height . 'px" class="repeattd t01"><tbody><div style=" text-align: center; padding-top: 55px;"> &nbsp; <br> </div></tbody></tbody></table></div>';
                                }
                            }
                            $html .= '</div>';
                            $html .= '</div>';
    ...
    public function getPendingOrdersCount()
    {
        ...
        if ($this->storeApiLogin == true) {
            ...
            try {
                ...
                $storeResult = $this->apiCall('Cart', 'orderIdFromStore', $filters);
                if ($storeResult) {
            ...
            if (!$error) {
                ...
                $msg = array('status' => 'failed', 'error' => 'Magento Store Error');
                $this->response($this->json($msg), 200);
            }
        }
    }
