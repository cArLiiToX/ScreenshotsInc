            ...
            foreach ($value->order_items as $line_key => $line_items) {
                ...
                if ($line_items->ref_id != '' && $line_items->ref_id != 0) {
                    ...
                    foreach ($json_content['sides'] as $key => $sides) {
                            ...
                            $html .= '<div style="padding: 0px 20px 20px 20px; width: 44%; text-align: center; background-color: #efefef; margin: 20px 0px 20px 13px; display:inline-block;' . $onesidewidth . '"><div class="m-b-7 barcode"><img style="' . $barcodewidth . '" src="data:image/png;base64,' . $product_barcode . '" height="50px" alt="" /></div><div style="margin-bottom:10px;"><h3>' . $base64 . '</h3></div><div style="margin-bottom:20px; min-height:400px; min-width:400px;"><img style="width:100%;" class="product-img" src="' . $pUrl . '" alt=""></div>';
