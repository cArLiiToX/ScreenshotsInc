...
class ProductsStore extends UTIL
{
    ...
    public function getSimpleProductClient()
    {
        ...
        $result = $this->datalayer->getSimpleProducts($pid, $confId, $color, $size);
        if (empty($result)) {
            ...
            $templates = array();
            if (isset($pid) && $pid) {
                $sql = "SELECT template_id FROM template_product_rel WHERE product_id = " . $pid;
            ...
            if (isset($pid) && $pid) {
                $sql = "SELECT ref_id,parent_id FROM " . TABLE_PREFIX . "template_state_rel WHERE temp_id = " . $pid;
    ...
    public function getSizeAndQuantity()
    {
        ...
        if ($this->storeApiLogin == true) {
            ...
            if (!$error) {
                try {
                    if (!$byAdmin) {
                        $result = $this->datalayer->getSizeAndQuantity($product_id, $varient_id, $color, $size);
                    } else {
                        $result = $this->datalayer->getSizeVariants($product_id, $varient_id, $color, $size);
                    }
                } catch (Exception $e) {
    ...
    public function getSimpleProduct()
    {
        ...
        $result = $this->storeApiLogin();
        if ($this->storeApiLogin == true) {
            $color_value = $this->getStoreAttributes("xe_color");
            $size_value = $this->getStoreAttributes("xe_size");
            ...
            if (!$error) {
                try {
                    ...
                    $result = $this->datalayer->getSimpleProducts($id, $confId, $color_value, $size_value);
                    if (empty($result)) {
                        $result = json_encode(array('No Records Found'));
                        $error = true;
                    } else {
...
// chk for storeid
    ...
    public function getSizeArr()
    {
        ...
        if ($this->storeApiLogin == true) {
            try {
                $size = $this->getStoreAttributes("xe_size");
                $result = $this->datalayer->getSizeArr($size);
            } catch (Exception $e) {
                $result = json_encode(array('isFault' => 1, 'faultMessage' => $e->getMessage()));
                $error = true;
            }
    ...
    public function getColorArr($isSameClass = false)
    {
        ...
        if ($this->storeApiLogin == true) {
            try {
                $color = $this->getStoreAttributes("xe_color");
                $result = $this->datalayer->getColorArr($this->_request['lastLoaded'], $this->_request['loadCount'], $productid, $color);
                print_r($result);exit;
            } catch (Exception $e) {
                $result = json_encode(array('isFault' => 1, 'faultMessage' => $e->getMessage()));
                $error = true;
