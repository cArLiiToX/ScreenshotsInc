   ...
   public function createWithoutProductSvg($reqSvgFile, $order_id, $item_id, $resultDesign, $folderStr = 1)
    ...
    public function creteNameAndNumberSeparatedSvg($reqSvgFile, $jsondata, $order_id, $refids, $size, $folderStr = 1)
    {
        ...
        foreach ($jsondata['nameNumberData']['list'] as $k1 => $v1) {
            if (strtoupper($size) == strtoupper($v1['size'])) {
                $nnFolderName = 'NN_' . $i;
                ...
                foreach ($main as $k => $g) {
                    ...
                    if ($id == "layer_area_" . $k . "") {
                        if (strpos($main[$k], "xe_p")!==false){
                            ...
                            $oldStr = array('<fegaussianblur','<clippath','<fecolormatrix','</fegaussianblur','</clippath','</fecolormatrix','patternunits');
                            $replaceStr = array('<feGaussianBlur','<clipPath','<feColorMatrix','</feGaussianBlur','</clipPath','</feColorMatrix','patternUnits');
