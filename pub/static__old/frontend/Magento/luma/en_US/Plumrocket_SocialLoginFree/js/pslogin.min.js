require(['jquery'],function(pjQuery){'use strict';pjQuery(document).ready(function(){pjQuery(document).on('click','.pslogin-showmore-button',function(){var $buttons=pjQuery(this).parents('div.pslogin-buttons');$buttons.find('.pslogin-hidden').fadeToggle(275);pjQuery(this).parents('div.pslogin-showmore').hide();if(showFullButtonsAfterViewMore!=true){$buttons.removeClass('pslogin-buttons-showfull');$buttons.parents('div.pslogin-block.pslogin-login').find('.pslogin-title').show();}});pjQuery(document).on('click','.pslogin-button-click',function(){var $this=pjQuery(this);psLogin($this.data('href'),$this.data('width'),$this.data('height'));return false;});pjQuery('.pslogin-fake-email-message .close-message').on('click',function(){pjQuery(this).parent().hide();});});window.psLogin=function(href,width,height)
...
if(href){win=window.open(href,'pslogin_popup',params.join(','));win.focus();pjQuery(win.document).ready(function(){var loaderText='Loading...';var html='<!DOCTYPE html><html style="height: 100%;"><head><meta name="viewport" content="width=device-width, initial-scale=1"><title>'+loaderText+'</title></head>';html+='<body style="height: 100%; margin: 0; padding: 0;">';html+='<div style="text-align: center; height: 100%;"><div id="loader" style="top: 50%; position: relative; margin-top: -50px; color: #646464; height:25px; font-size: 25px; text-align: center; font-family: Verdana, Geneva, Arial, Helvetica, sans-serif;">'+loaderText+'</div></div>';html+='</body></html>';pjQuery(win.document).contents().html(html);});}else{alert('This Login Application was not configured correctly. Please contact our customer support.');}
return false;}});